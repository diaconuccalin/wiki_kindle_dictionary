...
```bash
# Pocket: 100K long-abstract entries (default)
make all                         # or: make all PROFILE=pocket

# Complete: all articles, long abstracts, split into 68 volumes
make complete                    # builds all 68 volumes
make complete VOLUME=1           # build a single volume
...
- **04_generate_html.py** — Generates batched XHTML content files, attribution page, cover image, and OPF package file. For the complete profile, splits entries into 68 volumes (defined in `volumes.py`) with per-volume directories, cover overlays, and OPF titles.
...
- `volumes.py` — Defines the 68 complete-profile volumes (letter ranges, digit splits, special chars) and provides `assign_volumes()` to bucket titles into volumes using bisect on start-key boundaries.
...
- **Two-tier abstract system:** Top-ranked articles (by pageviews) get full lead sections ("long"), the rest get first-paragraph-only ("short"). Tier cutoffs are in `config.py:PROFILES`. The complete profile uses `long_tier: None` (all articles get long abstracts).
...
| Profile | Long tier | Short tier | Total | Output |
|---|---|---|---|---|
| pocket | 100K | 0 | 100K | 1 .mobi (~55 MB) |
| complete | all | 0 | ~6.8M | 68 .mobi volumes |
